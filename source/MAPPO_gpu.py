...
class mappo_agent:
    def __init__(self, envs, args, env):
        ...
        if args.tie_critic_wts:
            self.critic = Critic(args.history, args.action_dim, self.n_agents, nactions = 19, global_state_net = self.global_state_critic).to(self.device)

        self.actors = [Actor(args.history, 19).to(self.device) if not args.tie_actor_wts else self.actor for i in range(self.n_agents)]
        self.critics = [Critic(args.history, args.action_dim, self.n_agents, nactions = 19).to(self.device) if not args.tie_critic_wts else self.critic for i in range(self.n_agents)]
            ...
            for step in range(self.args.nsteps):
                with torch.no_grad():
                    ...
                    values = [self.critics[agent](
                        ...
                        F.one_hot(actions, 19).float().to(self.device),
