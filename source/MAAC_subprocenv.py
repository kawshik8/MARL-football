...
class MAAC:
    ...
    def learn(self):
        ...
        i_t = 0
        for i in range(start, end, self.args.num_workers):
            ...
            for t in range(self.num_steps):
                ...
                if len(self.memory) > self.start_steps:
                    actions,_,_,_ = self.choose_actions(states)
            ...
            if not(len(self.memory) > self.start_steps and len(self.memory) > self.batch_size):
                ...
                print(states.shape, rewards.shape, dones.shape, actions.shape, next_states.shape)
            ...
            if not self.args.test_model and i % 25 < self.args.num_workers:
                ...
                torch.save({"states": self.memory.states, 
                            ...
                            "curr_capacity": self.memory.current_capacity}, args.save_dir + "memory.ckpt")
    ...
    def choose_actions(self, obs):
        ...
        if not self.args.test_model:
            ...
            return actions, actions, actions, actions
...
if __name__ == '__main__':
    ...
    #     p = Process(target=init_process, args=(rank, args.num_workers, run, args))
    #     p.start()
    #     processes.append(p)
